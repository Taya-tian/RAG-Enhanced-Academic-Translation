...
def use_only_language_search(lang, folder, rag_query_file_path):
    ...
    prompt_generator.generate_prompt_with_rag(reference_generator, rag_queries, lambda filename: folder / (f"{filename}_rag_prompt.txt" if lang is None else f"{filename}_{lang}_rag_prompt.txt"))
...
def main() -> None:
    ...
    for lang in ["cn", "en", None]:
        use_only_language_search(lang, folder, sys.argv[1:])
